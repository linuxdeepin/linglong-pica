...
do
    ...
    # 否则安装到$PREFIX目录
        ...
        ar -x "$file" "$data_file"
        # 解压data.tar文件到输出目录
        mkdir "$data_list_dir" || true
        tar -xvf "$data_file" -C "$data_list_dir" >> "/tmp/deb-source-file/$(basename "$file").list"
        rm "$data_file"

        # 清理不需要复制的目录
        rm -r "${data_list_dir:?}/usr/share/applications"* ||  true
        # 修改pc文件的prefix
        sed -i "s#/usr#$PREFIX#g" "$data_list_dir"/usr/lib/"$TRIPLET"/pkgconfig/*.pc || true
        sed -i "s#/usr#$PREFIX#g" "$data_list_dir"/usr/share/pkgconfig/*.pc || true
        # 修改指向/lib的绝对路径的软链接
